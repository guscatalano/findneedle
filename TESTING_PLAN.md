...
| U-A4 / U-B8 (ViewerSettings) | ◑ Partial | Extracted pure `NormalizeThreshold` + `ClampDetailsPanelHeight` helpers from `ResultsViewerSettings` and tested them (`ResultsViewerSettingsTests` — 9 tests). U-B7 full file round-trip deferred: the class is a static file-backed singleton; testing persistence needs a storage-path seam (would touch the real viewer-settings.json), a larger refactor than warranted now. |
| UX VM suite | ✓ Green | 29/29 ViewModel-category tests. |
