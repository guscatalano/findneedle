...
| EVTX open → results E2E | ✓ Done | `EvtxEndToEndTests` — real committed Sysmon `.evtx` parsed by the real `EVTXProcessor`, run through `FolderLocation`→`NuSearchQuery`→storage; asserts all 3 events load (raw + filtered) and the Sysmon provider is present. The pipeline-integration counterpart to the existing processor-level `TestGetResultsFromSampleFile`. |
| ETL inspector engine | ✓ Done | An "Inspect ETL" UI feature already existed (Settings → Inspect ETL → `InspectionService` → `EtlInfoExtractor` providers/build/sysinfo + tracerpt). Rather than duplicate it, the richer inspect (`Inspect(path)`→`EtlInfo`: Windows/OS build, pointer size, CPU count, capture window+duration, event count+lost, kernel-vs-manifest/TraceLogging format breakdown, providers with per-provider counts — all via TraceEvent, no tracefmt) was **merged into `EtlInfoExtractor`** (the engine the existing UI uses); the standalone `EtlInspector` was removed. Tested by `InspectEtl_ReportsBuildProvidersAndCounts` against a generated .etl. Also: real **OS build** read from the ETW header event's `ProviderVersion` (the header `OSVersion` is often `10.0.0.0`) → e.g. `10.0.26100` (`EtlBuildNumberTests`); and **copy-out** as plaintext/JSON/XML/CSV (`EtlInfoExtractor.To*`, `EtlInfoExportTests`, dialog "Copy as" row). Full **BuildLabEx** (e.g. `26100.3194.amd64fre.ge_release.240331-1435`) + parsed **branch** (`ge_release`), plus machine identity from the kernel SystemConfig rundown — **edition** (`Windows 10 Pro`), **install date**, **computer name** (`PORTARE`), **RAM** (32061 MB) — read from `SysConfig/BuildInfo` + `SystemConfig/CPU` (BuildLab falls back to a raw UTF-16 byte scan for traces without the kernel config rundown). Plus hardware: **GPU** (`AMD Radeon 780M`), **NICs + IPs/DNS**, **disk model** (`WD_BLACK SN850X`) from SystemConfig Video/NIC/PhyDisk, and **per-provider GUIDs**. All in the report + XML/CSV/JSON exports; asserted by `EtlBuildNumberTests`. |
