...
| Row scroll / load speed | ✓ Done | `RowScrollingPerformanceTests` (TestCategory `Performance`, local-only) — populates 50k rows in SQLite and times `GetPage` across initial load, a sequential top-down scroll, and deep jumps (the OFFSET-scan worst case). Measured: first page ~8 ms, sequential pages median ~0.5 ms, scroll-to-bottom (offset 49,900) ~2 ms — paging is not a bottleneck; only the initial populate is disk-bound. Asserts correctness + a generous per-page ceiling (won't flake), real numbers in console output. |
