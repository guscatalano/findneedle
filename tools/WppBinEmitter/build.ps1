...
& $tw -cfgdir:"$cfg" -odir:"$out" "$here\WppBinEmitter.cpp"
cmd /c "`"$vc`" >nul 2>&1 && cd /d `"$out`" && cl /nologo /EHsc /Zi /I `"$out`" /Fe:WppBinEmitter.exe `"$here\WppBinEmitter.cpp`" /link advapi32.lib > `"$out\cl_out.txt`" 2>&1"
if (Test-Path "$out\WppBinEmitter.exe") { Write-Output "EXE OK" } else { Write-Output "NO EXE"; Get-Content "$out\cl_out.txt" -Tail 15 }
