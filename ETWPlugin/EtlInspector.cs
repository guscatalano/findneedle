...
/// <summary>
/// A quick "what's in this .etl?" summary: OS/Windows build, machine shape, capture window, event
/// counts, a manifest/kernel format breakdown, and the providers present (with per-provider counts).
/// Reads the ETW header for the cheap fields and a single pass over the events for the rest.
/// </summary>
...
/// <summary>
/// Inspects an .etl using the TraceEvent library (no tracefmt needed). Works for kernel, manifest,
/// and TraceLogging/self-describing traces.
/// </summary>
public static class EtlInspector
{
    public static EtlInfo Inspect(string etlPath)
    {
        ...
        // AllEvents = every dispatched event (the total + provider tally). Kernel.All / Dynamic.All
        // fire additionally for their subsets, giving the format breakdown without double-counting
        // the total (each uses its own counter).
        source.AllEvents += e =>
        {
            total++;
        ...
        // Order providers by frequency for display.
